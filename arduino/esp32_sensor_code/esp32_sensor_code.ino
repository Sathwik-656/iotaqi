 ...
 * Sensors: MQ135 (Air Quality), MQ6 (LPG/Butane Gas), DHT11 (Temperature & Humidity)
 * Backend: Firebase Realtime Database
 * 
 * Pin Configuration:
 * MQ135 - Analog Pin (GPIO 34)
 * MQ6   - Analog Pin (GPIO 39)
 * DHT11 - Digital Pin (GPIO 4)
 */
...
const char* firebaseAuth = ""; // Leave empty for public write access

// Dashboard API Endpoint (optional - leave as is or update with your domain)
...
#define MQ6_PIN 39      // Analog pin for MQ6 (LPG/Butane Gas)
...
const float MQ6_CLEAN_AIR_RATIO = 10.0;   // Clean air ratio for MQ6 (LPG/Butane)
...
SensorData readSensors() {
  ...
  // Read MQ6 (LPG/Butane Gas level)
  int mq6Raw = analogRead(MQ6_PIN);
  float mq6Voltage = (mq6Raw / 4095.0) * 3.3;
  float mq6Ratio = mq6Voltage / (3.3 - mq6Voltage);
  data.lpgLevel = calculateLPG(mq6Ratio);
...
int calculateLPG(float ratio) {
  // Calculate LPG concentration from MQ6 sensor ratio
  // MQ6 is sensitive to LPG, butane, propane
  // Formula based on MQ6 datasheet: Rs/R0 vs ppm relationship
  float ppm = 1000 * pow(ratio / MQ6_CLEAN_AIR_RATIO, -1.5);
  return constrain((int)ppm, 200, 10000); // Typical LPG detection range
}
...
void printSensorData(SensorData data) {
  ...
  Serial.println("LPG Level: " + String(data.lpgLevel) + " ppm");
